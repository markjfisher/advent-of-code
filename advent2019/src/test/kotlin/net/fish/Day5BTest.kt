...
class Day5BTest {
    ...
    fun `day 5 machine tests - around 8`() {
        ...
        val machine1 = AdventComputer(memory = around8Program.toMutableList(), inputs = listOf(0)).runProgram()
        assertThat(machine1.outputs).hasSize(1)
        assertThat(machine1.outputs.first()).isEqualTo(999)

        val machine2 = AdventComputer(memory = around8Program.toMutableList(), inputs = listOf(8)).runProgram()
        assertThat(machine2.outputs).hasSize(1)
        assertThat(machine2.outputs.first()).isEqualTo(1000)

        val machine3 = AdventComputer(memory = around8Program.toMutableList(), inputs = listOf(10)).runProgram()
