...
class Day3B : Day3Base() {
    companion object {
        @JvmStatic
        fun main(args: Array<String>) {
            val wirePaths = Helpers.loadWireData("/day3-input.txt")
            val sd: Int = Day3B().calculateMinimumSignalDelay(wirePaths)
            println(sd) // 6084
        }
    }

    fun calculateMinimumSignalDelay(paths: Pair<List<String>, List<String>>): Int {
        ...
        val stepsToIntersection = findIntersections(wire1Points, wire2Points).map { intersection ->
            stepsTo(intersection, wire1Points) + stepsTo(intersection, wire2Points)
        }
        return stepsToIntersection.min() ?: 0
    }

    fun stepsTo(intersection: Pair<Int, Int>, points: List<Pair<Int, Int>>): Int {
