...
class Day8A: Day8Base() {
    companion object {
        @JvmStatic
        fun main(args: Array<String>) {
            val pixelData = Helpers.loadResourceAsIntList("/day8-input.txt")
            val image = Image.readPixelData(pixelData, 25, 6)
            val minLayer = image.layers.minBy { it.count(0) }!!
            println(minLayer.count(1) * minLayer.count(2)) // 2125
        }
    }
}