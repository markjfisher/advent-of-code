...
if any, are called the instruction's parameters. For example, in the instruction 1,2,3,4,
1 is the opcode; 2, 3, and 4 are the parameters. The instruction 99 contains only an opcode and has no parameters.

The address of the current instruction is called the instruction pointer; it starts at 0.
After an instruction finishes, the instruction pointer increases by the number of values in the instruction; until
you add more instructions to the computer, this is always 4 (1 opcode + 3 parameters) for the add and multiply
    ...
    fun main(args: Array<String>) {
        ...
        for (noun in 0..99) {
            for (verb in 0..99) {
                ...
                val machine = Day2Machine(memory = initialMemory, instructionPointer = 0).runProgram()
                if (machine.memory[0] == 19690720) {
                    println("noun: $noun, verb: $verb, sum: ${100 * noun + verb}") // 8609
                    return
                }
            }
        }
