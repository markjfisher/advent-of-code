...
object Day19 : Day {
    ...
    private fun matches(ruleMap: Map<Int, String>, line: String, rules: List<Int>): Boolean {
        if (line.isEmpty() && rules.isEmpty()) {
            logger.debug { "line finished with no rules left, matched" }
            return true
        }

        if (line.isNotEmpty() && rules.isEmpty()) {
            logger.debug { "ran out of rules to match line, no match" }
            return false
        }

        // get next rule, and check it matches the current input, recursing like hell
        return ruleMap.getValue(rules[0]).let { rule ->
            logger.debug { "looking at rule: $rule (rules: $rules, line: $line)" }
            when {
                rule[0] in 'a'..'b' -> {
                    logger.debug { "got $rule" }
                    if (line.startsWith(rule[0])) {
                        val lineLess1st = line.drop(1)
                        val rulesLess1 = rules.drop(1)
                        logger.debug { "starts with $rule, going around dropping a char and rule: $lineLess1st, $rulesLess1" }
                        matches(ruleMap, lineLess1st, rulesLess1)
                    } else {
                        logger.debug { "line does not match, exiting check for $line\n" }
                        false
                    }
                }
                else -> rule.split(" | ").any { part ->
                    // Replace the current rule with the list of rules in this part of the OR clause, and recurse
                    val nextRules = part.split(" ").map(String::toInt) + rules.drop(1)
                    logger.debug { "not a/b: line: $line, part: $part, rules: $nextRules" }
                    matches(ruleMap, line, nextRules)
                }
            }
        }
    }
...
rules so you can determine which are valid and which are corrupted.
...
unmatched characters in the message. (For example, aaaabbb might appear to match
