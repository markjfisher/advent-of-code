...
object Day05 : Day {
    ...
    fun toBinary(v: String) = v.replace('F', '0').replace('B', '1').replace('L', '0').replace('R', '1').toInt(2)

    fun String.toSeatId() = toBinary(substring(0, 7)) * 8 + toBinary(substring(7))
    fun findSeatId(seatIds: List<Int>) = seatIds.windowed(2, 1).first { it[0] != it[1] - 1 }.first() + 1
...
seat is yours, and all of the flight attendants are busy with the flood of people that suddenly made it through passport control.
