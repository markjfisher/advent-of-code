...
object Day22 : Day {
    ...
    fun toGame(data: List<String>): Game {
        ...
        check(playerCards.count() == 2)
        return Game(playerCards[0], playerCards[1])
    }

    override fun part1() = doPart1(game)
    override fun part2() = doPart2(game)

    fun doPart1(game: Game): Long {
        return game.playEasy().score()
    }
...
data class Game(
    ...
    fun playEasy(): Game {
        return competeEasy(player1, player2)
    }

    fun playRecursed(): Game {
//        logger.debug { "=== Game $gameNumber ===" }
        return competeRecursed(player1 = player1, player2 = player2, round = 1)
    }

    fun score(): Long {
        if (player1.isNotEmpty() && player2.isNotEmpty() && winner == 0) throw Exception("Neither player has win. No score")
        val order = if (winner == 1 || player1.isNotEmpty()) player1 else player2
        val points = (order.count() downTo 1).toList()
        return order.zip(points).fold(0L) { sum, pair ->
            sum + pair.first.toLong() * pair.second.toLong()
        }
    }

    fun competeEasy(player1: List<Int>, player2: List<Int>): Game {
        if (player1.isEmpty() || player2.isEmpty()) return Game(player1, player2)

        val p1 = player1.first()
        val p2 = player2.first()
        return when {
            p1 > p2 -> competeEasy(player1.drop(1) + listOf(p1, p2), player2.drop(1))
            else -> competeEasy(player1.drop(1), player2.drop(1) + listOf(p2, p1))
        }
    }

    tailrec fun competeRecursed(player1: List<Int>, player2: List<Int>, round: Int, history1: MutableSet<List<Int>> = mutableSetOf(), history2: MutableSet<List<Int>> = mutableSetOf()): Game {
        if (history1.contains(player1) && history2.contains(player2)) {
//            logger.debug { "seen this game before for player1: ${history1.contains(player1)}, player2: ${history2.contains(player2)}" }
            return Game(player1 = emptyList(), player2 = emptyList(), gameNumber = gameNumber, parentGame = parentGame, winner = 1)
        }
...
//                                ...anyway, back to game $gameNumber.
//                            """.trimIndent())
//                    logger.debug { "Player 2 wins round $round of game $gameNumber!" }
            ...
            when {
                p1 > p2 -> {
//                    logger.debug { "Player 1 wins round $round of game $gameNumber!" }
                    competeRecursed(player1 = player1.drop(1) + listOf(p1, p2), player2.drop(1), round + 1, history1, history2)
                }
                else -> {
//                    logger.debug { "Player 2 wins round $round of game $gameNumber!" }
                    competeRecursed(player1.drop(1), player2.drop(1) + listOf(p2, p1), round + 1, history1, history2)
                }
            }
        }
    }
...
player with the higher-valued card wins the round. The winner keeps both cards, placing
them on the bottom of their own deck so that the winner's card is above the other card.
...
As in regular Combat, the winner of the round (even if they won the round by winning a sub-game) takes the
two cards dealt at the beginning of the round and places them on the bottom of their own deck (again so that
the winner's card is above the other card). Note that the winner's card might be the lower-valued of the two
...
Recursive Combat. (For example, if player 1 draws the 3 card, and player 2 draws the 7 card, this would
occur if player 1 has at least 3 cards left and player 2 has at least 7 cards left, not counting the
3 and 7 cards that were drawn.)
...
players' decks to form the sub-game. (For example, if player 1 drew the 3 card, their deck in the sub-game would
