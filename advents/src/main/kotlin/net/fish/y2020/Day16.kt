...
object Day16 : Day {
    ...
    fun doPart1(rules: List<Rule>, otherTicketsList: List<List<Int>>): Int {
        val otherTicketsP1 = otherTicketsList.flatten()
        val validIds = rules.fold(mutableSetOf<Int>()) { acc, rule -> acc.addAll(rule.toValidIds()); acc }
        // don't use subtract, it removes duplicates in the values left
        val invalidIds = otherTicketsP1.filterNot { validIds.contains(it) }
        return invalidIds.sum()
    }

    fun doPart2(rules: List<Rule>, myTicket: List<Int>, otherTickets: List<List<Int>>): Long {
        val columnToRule = createColumnToRuleMapping(rules, otherTickets)
        val matchingColumnIds = columnToRule.filter { it.value.name.startsWith("departure") }.map { it.key }
        return matchingColumnIds.fold(1L) { acc, i -> acc * myTicket[i].toLong() }
    }

    fun createColumnToRuleMapping(rules: List<Rule>, otherTickets: List<List<Int>>): Map<Int, Rule> {
        val validIds = rules.fold(mutableSetOf<Int>()) { acc, rule -> acc.addAll(rule.toValidIds()); acc }
        ...
        // This is brittle if the generic case didn't have 1 more rule matching every column, but with the puzzle data it's fine.
        val columnToRule = mapColumnNumberToRules.entries.sortedBy { it.value.count() }.fold(mutableMapOf<Int, Rule>()) { acc, mutableEntry ->
            val (columnNumber, rulesList) = mutableEntry
            val foundRules = acc.values.toList()
            acc[columnNumber] = (rulesList - foundRules).first()
            acc
        }

        return columnToRule.toMap()
    }

    fun toRules(data: List<String>): List<Rule> {
        val ruleExtractor = Regex("""^([a-z ]+): (\d+)-(\d+) or (\d+)-(\d+)$""")
        return data.map { ruleLine ->
            ruleExtractor.find(ruleLine)?.destructured!!.let { (name, l1, l2, h1, h2) ->
                Rule(name, l1.toInt(), l2.toInt(), h1.toInt(), h2.toInt())
            }
        }
    }
    ...
    data class Rule(
        ...
        fun toValidIds(): Set<Int> {
            return mutableSetOf<Int>().also {
                it.addAll(IntRange(low1, low2))
                it.addAll(IntRange(high1, high2))
            }
        }

        fun matches(values: List<Int>): Boolean = toValidIds().containsAll(values)
    }
...
in every ticket is named class and can be any value in the ranges 1-3 or 5-7 (inclusive, such that 3 and 5
...
This ticket might be represented as 101,102,103,104,301,302,303,401,402,403; of course, the actual train
tickets you're looking at are much more complicated. In any case, you've extracted just the numbers in
such a way that the first number is always the same specific field, the second number is always a different
...
nearby ticket are all valid for at least one field.
