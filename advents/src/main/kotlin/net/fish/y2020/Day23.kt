...
object Day23 : Day {
    ...
    fun doPart1(data: List<Int>): String {
        ...
        var cup = cups[1]!!.next!!
        return (0 until (data.size - 1)).map {
            val v = cup.value
            cup = cup.next!!
            v
        }.joinToString("")
    ...
    fun doPart2(data: List<Int>): Long {
        val cups = pjAlgorithm(data, 1_000_000, 10_000_000)
        val cup1 = cups[1]!!
        val next1 = cup1.next!!
        val next2 = next1.next!!
        return next1.value.toLong() * next2.value.toLong()
    }

    fun pjAlgorithm(data: List<Int>, cupCount: Int, iterations: Int): Map<Int, SimpleLinkedNode> {
        val cups = createCups(data, cupCount)
        solve(cups[data.first()]!!, cupCount, cups, iterations)
        return cups
    }

    fun createCups(data: List<Int>, cupCount: Int): Map<Int, SimpleLinkedNode> {
        val cups = mutableMapOf<Int, SimpleLinkedNode>()
        val head = SimpleLinkedNode(data.first())
        var next = head
        cups[head.value] = head

        data.drop(1).forEach { cup ->
            val node = SimpleLinkedNode(cup)
            next.next = node
            next = node
            cups[cup] = node
        }
        ...
        if (cupCount > 9) {
            (10..cupCount).forEach { i ->
                val node = SimpleLinkedNode(i)
                next.next = node
                next = node
                cups[i] = node
            }
    ...
    fun solve(head: SimpleLinkedNode, cupsCount: Int, cups: Map<Int, SimpleLinkedNode>, iterations: Int): Map<Int, SimpleLinkedNode> {
        ...
        repeat(iterations) { iteration ->
            ...
            val next3 = setOf(removed.value, removed.next!!.value, removed.next!!.next!!.value)
            while (next3.contains(destination)) {
                destination--
                if (destination == 0) destination = cupsCount
            }
            val insertNode = cups[destination]!!
...
class SimpleLinkedNode(
    val value: Int,
    var next: SimpleLinkedNode? = null
)
...
going clockwise around the circle from the first cup, the cups would be labeled
...
Your puzzle input is 476138259.
 */
