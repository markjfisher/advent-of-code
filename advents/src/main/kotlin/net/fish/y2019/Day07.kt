...
object Day07: Day {
    ...
    fun calculateThrust(initialState: List<Long>, sequence: List<Long>): Long {
        ...
        val outB = AdventComputer(initialState.toMutableList(), listOf(sequence[1], outA)).run().out()
        val outC = AdventComputer(initialState.toMutableList(), listOf(sequence[2], outB)).run().out()
        val outD = AdventComputer(initialState.toMutableList(), listOf(sequence[3], outC)).run().out()
        return AdventComputer(initialState.toMutableList(), listOf(sequence[4], outD)).run().out()

    }

    fun calculateThrustFeedback(initialState: List<Long>, sequence: List<Long>): Long {
        val machineA = AdventComputer(initialState.toMutableList(), listOf(sequence[0], 0)).run()
        val machineB = AdventComputer(initialState.toMutableList(), listOf(sequence[1], machineA.out())).run()
        val machineC = AdventComputer(initialState.toMutableList(), listOf(sequence[2], machineB.out())).run()
        val machineD = AdventComputer(initialState.toMutableList(), listOf(sequence[3], machineC.out())).run()
