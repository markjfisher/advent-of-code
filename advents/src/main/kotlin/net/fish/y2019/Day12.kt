...
object Day12: Day {
    ...
    override fun part1(): Int {
        runSimulation(bodies.toMutableList())
        return bodies.map { it.total }.sum()
    }

    override fun part2() = findCycleLength()

    private fun findCycleLength(): Long {
        ...
        val bodies = bodies.toMutableList()
        // Only try to add history if they haven't yet cycled. Cuts time by half by not interacting with Set
        var hasXCycled = false
        var hasYCycled = false
        var hasZCycled = false
        while (true) {
            runSimulation(bodies, 1)
            val xAdded = !hasXCycled && historyX.add(bodies.map { body -> body.position.x to body.velocity.x })
            val yAdded = !hasYCycled && historyY.add(bodies.map { body -> body.position.y to body.velocity.y })
            val zAdded = !hasZCycled && historyZ.add(bodies.map { body -> body.position.z to body.velocity.z })
            if (!xAdded && !yAdded && !zAdded) {
                break
            }
            hasXCycled = !xAdded
            hasYCycled = !yAdded
        ...
        return lcm(historyX.size.toLong(), lcm(historyY.size.toLong(), historyZ.size.toLong()))
    }


    fun runSimulation(bodies: MutableList<Body>, steps: Int = 1000) {
    ...
    private fun applyGravity(bodies: MutableList<Body>) {
        asPairs(bodies).forEach { (body1, body2) ->
            val dx = diff(body1.position.x, body2.position.x)
            val dy = diff(body1.position.y, body2.position.y)
            val dz = diff(body1.position.z, body2.position.z)

            body1.velocity += Point3D(dx, dy, dz)
            body2.velocity += Point3D(-dx, -dy, -dz)
        }
    }

    private fun applyVelocity(bodies: MutableList<Body>) {
        bodies.forEach { body ->
            body.position += body.velocity
        }
    }

    private fun diff(a: Int, b: Int): Int = if (a < b) 1 else if (a > b) -1 else 0

    private fun asPairs(bodies: MutableList<Body>) = bodies.toSet().combinations(2).map { it.zipWithNext() }.flatten()

    data class Point3D(val x: Int, val y: Int, val z: Int) {
        operator fun plus(other: Point3D) = Point3D(x + other.x, y + other.y, z + other.z)
    }

    data class Body(var position: Point3D, var velocity: Point3D = Point3D(0, 0, 0)) {
