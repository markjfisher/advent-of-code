...
internal class AdventCalendarTest {
    ...
    fun `8 test`() {
        ...
        val m1 = AdventComputer(program = around8Program.toMutableList(), initialInput = listOf(0)).run()
        assertThat(m1.outputs).hasSize(1)
        assertThat(m1.outputs.first()).isEqualTo(999)

        val m2 = AdventComputer(program = around8Program.toMutableList(), initialInput = listOf(8)).run()
        assertThat(m2.outputs).hasSize(1)
        assertThat(m2.outputs.first()).isEqualTo(1000)

        val m3 = AdventComputer(program = around8Program.toMutableList(), initialInput = listOf(10)).run()
